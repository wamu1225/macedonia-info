...
const FIGURE_DATA: Record<string, { caption: string; inner: string }> = {
  'phalanx': {
    caption: 'サリサとファランクスの模式図。長槍サリサ（約5.5から6メートル）を構えた兵が密集し、後列の槍も前方に届いて、幾重もの槍ぶすまをつくりました。図は仕組みを示すもので、隊列の人数や寸法は正確ではありません。',
    inner: `<div class="diagram-wrap">${phalanxSvg()}</div>`,
  },
  'campaign': {
    caption: '東方遠征の道すじと四つの決戦の地図（北が上）。西のマケドニア（ペラ）を発ち、グラニコス・イッソスで勝ち、南のエジプトへ回ったのち、ガウガメラでアケメネス朝を破り、東のヒュダスペス（インダス川の支流）にまで達しました。◆が決戦の地、●がおもな都市で、位置はおおよその実地点。海岸線は Natural Earth のデータに基づきます。',
    inner: `<div class="diagram-wrap">${campaignSvg()}</div>`,
  },
  'kingdoms': {
    caption: 'ディアドコイの争いののちに分かれたヘレニズム三王国の地図（北が上）。マケドニアを**アンティゴノス朝**、エジプトを**プトレマイオス朝**、シリアからペルシアにかけての西アジアを**セレウコス朝**が治めました。海岸線は Natural Earth のデータに基づき、色分けした範囲は勢力のおおよその広がりで、正確な国境ではありません。',
    inner: `<div class="diagram-wrap">${kingdomsSvg()}</div>`,
  },
  'greece': {
    caption: 'マケドニアと南部ギリシャの位置図（北が上）。マケドニアはエーゲ海の北に位置し、王家の都ペラとアイガイ（現在のヴェルギナ）は海に近い平野にありました。南のペロポネソス半島やアッティカには、アテナイ・スパルタ・テーバイ・アルゴス・コリントスといったポリスが並びます。王家が起源を主張したアルゴスも、王が参加を望んだ競技の地オリュンピアも、この南部にあります。海岸線は Natural Earth のデータに基づき、都市の位置は実際の座標によります。当時の勢力の境は示していません。',
    inner: `<div class="diagram-wrap">${greeceSvg()}</div>`,
  },
  'balkans': {
    caption: '現代の北マケドニア共和国と近隣国の位置図（北が上）。内陸のこの国は、北のセルビアとコソボ、東のブルガリア、南のギリシャ、西のアルバニアに囲まれています。金の点が首都スコピエ、南西の青い点が世界遺産のオフリド湖。国境は Natural Earth のデータに基づきます。',
    inner: `<div class="diagram-wrap">${balkansSvg()}</div>`,
  },
  'rulers': {
    caption: 'マケドニアの地を治めた勢力の移り変わりを示した帯年表（北が上ではなく、左から右へ時間が進む）。帯の幅は、その支配がおよそどれだけ続いたかを表します。ローマ帝国からその流れをくむ東ローマ（ビザンティン）帝国までが千年あまりを占め、6から7世紀にはスラヴ人の定住が進んだ。中世にはブルガリアやセルビアなどの勢力が入れ替わり、およそ1400年ごろから1912年まで、五百年ほどオスマン帝国の統治が続きました。年の区切りはおおよそのもの。',
    inner: `<div class="diagram-wrap">${rulersSvg()}</div>`,
  },
  'vergina': {
    caption: 'ヴェルギナの王墓の模式図。大きな墳丘の下に王墓が隠され、黄金の納骨箱（ラルナクス）が納められていました。蓋にはアルゲアス朝の象徴とされる星（太陽）の意匠が描かれていました。図は仕組みを示すもので、光線の数や寸法は正確ではありません。',
