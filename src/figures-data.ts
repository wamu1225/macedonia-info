...
const FIGURE_DATA: Record<string, { caption: string; inner: string }> = {
  'phalanx': {
    caption: 'サリサとファランクスの模式図。長槍サリサ（約5.5から6メートル）を構えた兵が密集し、後列の槍も前方に届いて、幾重もの槍ぶすまをつくった。図は仕組みを示すもので、隊列の人数や寸法は正確ではありません。',
    inner: `<div class="diagram-wrap">${phalanxSvg()}</div>`,
  },
  'campaign': {
    caption: '東方遠征の道すじと四つの決戦の地図（北が上）。西のマケドニア（ペラ）を発ち、グラニコス・イッソスで勝ち、南のエジプトへ回ったのち、ガウガメラでアケメネス朝を破り、東のヒュダスペス（インダス川の支流）にまで達しました。◆が決戦の地、●がおもな都市で、位置はおおよその実地点。海岸線は Natural Earth のデータに基づく。',
    inner: `<div class="diagram-wrap">${campaignSvg()}</div>`,
  },
  'kingdoms': {
    caption: 'ディアドコイの争いののちに分かれたヘレニズム三王国の地図（北が上）。マケドニアを**アンティゴノス朝**、エジプトを**プトレマイオス朝**、シリアからペルシアにかけての西アジアを**セレウコス朝**が治めた。海岸線は Natural Earth のデータに基づき、色分けした範囲は勢力のおおよその広がりで、正確な国境ではありません。',
    inner: `<div class="diagram-wrap">${kingdomsSvg()}</div>`,
  },
  'greece': {
    caption: 'マケドニアと南部ギリシャの位置図（北が上）。マケドニアはエーゲ海の北に位置し、王家の都ペラとアイガイ（現在のヴェルギナ）は海に近い平野にあった。南のペロポネソス半島やアッティカには、アテナイ・スパルタ・テーバイ・アルゴス・コリントスといったポリスが並ぶ。王家が起源を主張したアルゴスも、王が参加を望んだ競技の地オリュンピアも、この南部にあります。海岸線は Natural Earth のデータに基づき、都市の位置は実際の座標による。当時の勢力の境は示していません。',
    inner: `<div class="diagram-wrap">${greeceSvg()}</div>`,
  },
  'balkans': {
    caption: '現代の北マケドニア共和国と近隣国の位置図（北が上）。内陸のこの国は、北のセルビアとコソボ、東のブルガリア、南のギリシャ、西のアルバニアに囲まれています。金の点が首都スコピエ、南西の青い点が世界遺産のオフリド湖。国境は Natural Earth のデータに基づく。',
