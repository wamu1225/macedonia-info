...
// 4) 内部リンク [/macedonia-info/PATH/] の PATH が実在する記事ID（または about/privacy）であること
import fs from 'fs';
...
const expected = ['argead', 'philip', 'alexander', 'successors', 'archaeology', 'medieval', 'modern', 'faq'];
...
for (const { id, block } of blocks) {
  ...
  for (const lk of [...content.matchAll(/\]\(\/macedonia-info\/([a-z0-9-]+)\//g)]) {
    const target = lk[1];
